---
title: "Here's How BuzzFeed News Trained A Computer To Search For Hidden Spy Planes"
...
Data and [R](https://www.r-project.org/) code for the analysis supporting this [August x, 2017 BuzzFeed News post](https://www.buzzfeed.com/peteraldhous/hidden-spy-planes) on identifying candidate surveillance aircraft. Supporting files are in [this GitHub repository](https://github.com/BuzzFeedNews/2017-08-spy-plane-finder).
...
For the machine learning, we selected the [random forest](http://www.datasciencecentral.com/profiles/blogs/random-forests-algorithm) algorithm, popular among data scientists for classification tasks. 
...
This chart shows that the `steer1` and `steer2` variables, quantifying the frequency of turning hard to the left, and  `squawk_1`, the most common squawk code broadcast by a plane's transponder, were the most important.
...
This output shows that overall, the estimated classification error rate was 3.7%. For `other` planes, the estimated error rate was vanishingly low. However for the target `surveil` class, representing likely surveillance aircraft, the estimated error rate was 20.6%. We experimented with simplifying the model, removing some of the less important import variables, but that did not appreciably improve its performance.

We then used the model to classify all of the planes in the data, after first removing the planes used in training data and a list of known federal law enforcement aircraft, which are in the file `feds.csv`.
...
Before exporting, we joined to data from the FAA registration database, to include where available the planes' registration numbers and the organizations they are registered to.

```{r, results="hide", warning=FALSE, message=FALSE}
# load and process FAA registration data (here using download from Jul 25, 2017, to reflect current registration)

faa <- read_csv("data/ReleasableAircraft/master.csv") %>%
  select(1,7,34)
